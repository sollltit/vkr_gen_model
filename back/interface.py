...
from back.app import generate_answer
            ...
            with st.chat_message('assistant'):
                with st.spinner('Думаю...'):
                    ...
                    try:
                        ...
                        # берём ВСЮ историю БЕЗ последнего user (он уже добавлен)
                        history = current_chat["messages"][:-1]

                        messages.extend(history)
                        messages.extend([
                                m for m in current_chat["messages"]
                                if m["role"] != "system"
                            ])

                        # добавляем только текущий prompt
                        messages.append({"role": "user", "content": prompt})

                        full_response = generate_answer(clean_messages(messages))
